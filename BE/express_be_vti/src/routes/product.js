...
router.get("/", async (req, res, next) => {
  let { page, limit, search, categoryId, sortedProduct } = req.query;
  try {
    // Lấy dữ liệu sản phẩm
    let result = await new Promise((resolve, reject) => {
      productService.getProducts(
        { page, limit, search, categoryId, sortedProduct },
        (err, result) => {
          if (err) {
            reject(err);
          } else {
    ...
    // Lấy tổng số sản phẩm và thông tin về số lượng sản phẩm theo categoryId
    const total = await productService.getTotalProduct(search, categoryId);
    const count = await productService.getProductCountByCategory();
    ...
    res.send({
      ...
      metadata: {
        ...
        page,
        limit,
        count, // Thêm thông tin này vào metadata
      },
    });
  } catch (err) {
    next(err);
